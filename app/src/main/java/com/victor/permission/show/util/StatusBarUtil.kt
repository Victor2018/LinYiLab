...
private const val TAG = "StatusBarUtil"
    ...
    fun adaptationNav(activity: Activity?) {
        ...
        val navHeight = getNavigationBarHeight(activity)
        var contentView: View? = getContentView(activity)

        var paddingBottom = contentView?.paddingBottom
        Log.e(TAG,"adaptationNav-paddingBottom = $paddingBottom")
        if (paddingBottom == 0) {
    ...
    fun getContentView (activity: Activity?): View? {
        ...
        var decorView = activity?.window?.decorView?.findViewById<View>(android.R.id.content) as ViewGroup
    ...
    fun translucentStatusBar(activity: Activity, hideStatusBarBackground: Boolean,
                             statusTextColorBlack: Boolean, navigationBarTranslucent: Boolean) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ...
                if (statusTextColorBlack) {
                    ...
                    //需要在layout根布局中添加android:fitsSystemWindows="true"
...
//                    window.setNavigationBarColor(activity.resources.getColor(R.color.white))
                }
            }
        }
    }
    ...
    fun isNavigationBarExist(activity: Activity): Boolean {
        var NAVIGATION = "navigationBarBackground"

        var vp: ViewGroup = activity.getWindow().getDecorView() as ViewGroup
        if (vp != null) {
            for (i in 0 until vp.getChildCount()) {
